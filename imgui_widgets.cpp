...
extern ImFont* ico_Font;
extern ImFont* Cyrcle_Font;
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    RenderFrame(bb.Min, bb.Max, ImColor(175, 126, 164, 63), true, 5.f);

    GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + (size_arg.x - CalcTextSize(label).x) / 2, bb.Max.y - CalcTextSize(label).y - (size.y - CalcTextSize(label).y) / 2), ImColor(175, 126, 164, 255), label);
...
bool ImGui::Button1(const char* label, const char* ico, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, ImColor(175, 126, 164, 255), true, 5.f);

    GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + (size_arg.x - CalcTextSize(label).x) / 2 - 10, bb.Max.y - CalcTextSize(label).y - (size.y - CalcTextSize(label).y) / 2), ImColor(11, 11, 11, 255), label);
    GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + (size_arg.x - CalcTextSize(ico).x) / 2 + 24, bb.Max.y - CalcTextSize(ico).y - (size.y - CalcTextSize(ico).y) / 2), ImColor(11, 11, 11, 255), ico);
...
bool ImGui::ButtonCyrcle(const char* label, const char* icos, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    static std::map<ImGuiID, ButtonCyrcle_state> anim;
    auto it_anim = anim.find(id);

    if (it_anim == anim.end())
    {
        anim.insert({ id, ButtonCyrcle_state() });
        it_anim = anim.find(id);
    }

    it_anim->second.color = ImLerp(it_anim->second.color, IsItemActive() ? ImColor(11, 11, 11, 255) : hovered ? ImColor(11, 11, 11, 255) : ImColor(90, 90, 90), g.IO.DeltaTime * 10.f);

    it_anim->second.bg_color = ImLerp(it_anim->second.bg_color, IsItemActive() ? ImColor(175, 126, 164, 255) : hovered ? ImColor(175, 126, 164, 255) : ImColor(15, 15, 15), g.IO.DeltaTime * 10.f);
    // Render
    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    RenderNavHighlight(bb, id);
    // RenderFrame(ImVec2(bb.Min.x - 100, bb.Min.y - 100), ImVec2(bb.Max.x, bb.Max.y), ImColor(it_anim->second.bg_color), false, 150.f);
    window->DrawList->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), ImColor(it_anim->second.bg_color), 150.f);
   // window->DrawList->AddCircleFilled(ImVec2(bb.Min.x,bb.Min.y), 150.f, ImColor(it_anim->second.bg_color));

    PushFont(Cyrcle_Font);
    ImGui::GetForegroundDrawList()->AddText(ImVec2(bb.Max.x - CalcItemWidth() + 130, bb.Min.y + 70), ImColor(it_anim->second.color), icos);
    ImGui::PopFont();
...
extern ImFont* ico_Font1;

bool ImGui::ButtonIcons(const char* label,const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    PushFont(ico_Font1);
    GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + (size_arg.x - CalcTextSize(label).x) / 2 - 10, bb.Max.y - CalcTextSize(label).y - (size.y - CalcTextSize(label).y) / 2), ImColor(37, 37, 37, 255), label);
    PopFont();
...
bool ImGui::CircleAnim(const char* label)
{
    ...
    const ImVec2 centre = ImVec2(pos.x + 155.f, pos.y + 325.f + style.FramePadding.y);
    static ImColor fore_color(123, 58, 77, animantion);
    static ImColor back_color(123, 58, 77, animantion);
    ...
    front->PathClear();
    front->PathArcTo(centre, 30.f, 0.f, 2.f * IM_PI, 40.f);
    front->PathStroke(ImGui::GetColorU32(back_color.Value), 0, 5);
    front->PathClear();
    front->PathArcTo(centre, 30.f, IM_PI * 1.5f + position, IM_PI * (1.5f + arc_size) + position, 40.f);
    front->PathStroke(ImGui::GetColorU32(fore_color.Value), 0, 5);

    if (it_alpha->second > 100)
    {
        alpha_anim.end();
    }
...
bool ImGui::ButtonSellect(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    PushFont(Medium_Font);
    ImGui::GetForegroundDrawList()->AddText(ImVec2(bb.Min.x, bb.Min.y), ImColor(90,90,90), label);
    ImGui::PopFont();
...
bool ImGui::Tab(bool selected, const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    it_anim->second.inflating = ImLerp(it_anim->second.inflating, selected ? 0.f : 20.f, g.IO.DeltaTime * 6.f);

    it_anim->second.background = ImLerp(it_anim->second.background, selected ? ImColor(37, 37, 37, 127) : hovered ? ImColor(37, 37, 37, 127) : ImColor(37, 37, 37, 0), g.IO.DeltaTime * 6.f);
    //it_anim->second.text = ImLerp(it_anim->second.text, selected ? colors::button::text_active : hovered ? colors::button::text_hov : colors::button::text, g.IO.DeltaTime * 6.f);

    RenderFrame(bb.Min + ImVec2(it_anim->second.inflating, it_anim->second.inflating), bb.Max - ImVec2(it_anim->second.inflating, it_anim->second.inflating), GetColorU32(it_anim->second.background), true, 8.f);
...
bool ImGui::ButtonSellected(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    window->DrawList->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), ImColor(37, 37, 37, 127), 8.f);
...
bool ImGui::InputText(const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    ...
    return InputTextEx(label, NULL, NULL, buf, (int)buf_size, ImVec2(0, 0), flags, callback, user_data);
}
...
bool ImGui::InputTextWithHint(const char* label, const char* hint, const char* icos, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    ...
    return InputTextEx(label, hint, icos, buf, (int)buf_size, ImVec2(0, 0), flags, callback, user_data);
...
bool ImGui::InputTextEx(const char* label, const char* hint, const char* icos, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth() - 10, (is_multiline ? g.FontSize * 8.0f : label_size.y + 17) + style.FramePadding.y * 7.0f); // Arbitrary default of 8 lines high for multi-line
    ...
    {
        RenderNavHighlight(frame_bb, id);
        it_anim->second.input_color = ImLerp(it_anim->second.input_color, IsItemActive() ? ImColor(20, 20, 20, 255) : hovered ? ImColor(20, 20, 20, 255) : ImColor(15, 15, 15), g.IO.DeltaTime * 8.f);
        const int vtx_idx_1 = ImGui::GetWindowDrawList()->VtxBuffer.Size;
        // ImGui::GetForegroundDrawList()->AddRect(ImVec2(frame_bb.Min.x - 1, frame_bb.Min.y + 4), ImVec2(frame_bb.Max.x + 1, frame_bb.Max.y - 4), ImColor(60, 60, 60, 255), true, 1.f);
        ImGui::GetForegroundDrawList()->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 5), ImVec2(frame_bb.Max.x - 106, frame_bb.Max.y - 5), GetColorU32(it_anim->second.input_color), 10.f);
        // ImGui::ShadeVertsLinearColorGradientKeepAlpha(ImGui::GetWindowDrawList(), vtx_idx_1, ImGui::GetWindowDrawList()->VtxBuffer.Size, frame_bb.Min, ImVec2(frame_bb.Max.x - CalcItemWidth() - 54, frame_bb.Max.y), ImColor(154, 0, 255, 255), ImColor(154, 0, 255, 255));
    ...
    if (render_cursor || render_selection)
    {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                        ImGui::GetForegroundDrawList()->AddRectFilled(rect.Min + ImVec2(4, 7), rect.Max + ImVec2(4, 7), bg_color);
                }
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            ImGui::GetForegroundDrawList()->AddText(g.Font, g.FontSize, draw_pos - draw_scroll + ImVec2(4, 15), col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect)) {
                ...
                ImGui::GetForegroundDrawList()->AddRectFilled(frame_bb.Min + ImVec2(it_anim->second.value + 2, 18), frame_bb.Min + ImVec2(it_anim->second.value + 3, 37), GetColorU32(ImGuiCol_Text));
            }
    ...
    else
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
           ...
           ImGui::GetForegroundDrawList()->AddText(g.Font, g.FontSize, draw_pos + ImVec2(4, 15), col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
    ...
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, IsItemActive() ? ImColor(255, 255, 255, 255) : hovered ? ImColor(255, 255, 255, 255) : ImColor(255, 255, 255, 255), g.IO.DeltaTime * 6.f);
    PushFont(Medium_Font);
    ImGui::GetForegroundDrawList()->AddText(ImVec2(frame_bb.Max.x - CalcItemWidth() + 20, frame_bb.Min.y - 20), ImColor(90, 90, 90, 255), label);
    ImGui::PopFont();
    PushFont(ico_Font);
    ImGui::GetForegroundDrawList()->AddText(ImVec2(frame_bb.Max.x - CalcItemWidth() + 270, frame_bb.Min.y + 18), ImColor(175, 126, 164, 255), icos);
    ImGui::PopFont();
